...
if(!empty($_POST['pseudo']) && !empty($_POST['email']) && !empty($_POST['password'])){
	...
	// Variables
	$pseudo = htmlspecialchars($_POST['pseudo']);  // on protege nos variables de l'injection SQL
	$email = htmlspecialchars($_POST['email']);
	$password = htmlspecialchars($_POST['password']);

	// Vérifier la syntaxe de l'email
	if(!Verifier::syntaxeEmail($email)){  // vérifier va renvoyer true si l'email est bon donc utiliser ! pour préciser l'inverse
		header('location: index.php?error=true&message=Veuillez vérifier le format de votre adresse email.');
		exit();
	}


	// Vérifier les éventuels doublons de l'email

	if(Verifier::doublonEmail($email)){   // renvoie true s'il y a un doublon sinon false donc ici pas besoin de !
